...
function validateDOB() {
    ...
    if (dobInput) {
        ...
        // Calculate the date for 120 years ago
        const maxDOB = new Date();
        maxDOB.setFullYear(today.getFullYear() - 120);

        // Check if the DOB is in the future or older than 120 years ago
        if (dob > today) {
        ...
        // If no date is entered, consider it an error
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (firstName) {
        ...
        addNewUserCheckbox(firstName);
        document.getElementById("fname").value = firstName;
    } else {
        heading.textContent = "Welcome New User";
    }
});


function addNewUserCheckbox(name) {
    ...
    newUserCheckbox.onclick = function() {
        if (newUserCheckbox.checked) {
            ...
            document.querySelector("h1").textContent = "Welcome New User";
        }
    };

    const newUserLabel = document.createElement("label");
    newUserLabel.htmlFor = "newUserCheckbox";
    newUserLabel.textContent = `Not ${name}? Click here to start as a new user.`;

    const headingContainer = document.querySelector("h1");
    headingContainer.parentNode.insertBefore(newUserCheckbox, headingContainer.nextSibling);
    headingContainer.parentNode.insertBefore(newUserLabel, newUserCheckbox.nextSibling);
}
